# assumes TACC data is loaded so that 
...
n_reps <- 30
# start doing a loop that's not a loop
for (i in 1:n_reps) {
  ...
  size_hist <- hist(rle_temp$values, 
                    breaks = seq(0, 16, 2), # go long - can truncate for plots
                    plot = FALSE
  )
  
  ###### make a histogram for lifetimes #####
  lifetime_hist <- hist(rle_temp$lengths, 
                        breaks = seq(0, 20, 2), # 0, 30, 0.2 go long - can truncate for plots
                        plot = FALSE
  )
...
} ### end of loop over bootstrap replicates
